...
const BlogCard = ({ blogdata }) => {
    ...
    return (
        <div className='bg-base-100 shadow-lg rounded p-4 md:p-9 my-4'>
            <div className='static md:flex justify-start'>
                <div className='relative w-full h-52 mx-auto'>
                    <img src={setImgUrl} className='h-full w-full' alt="blog photo" />
                </div>
                <div className='md:ml-7 mb-auto mt-5 '>
                    <Link href={`/blog/${_id}`}><a> <h2 className="card-title text-xl md:text-2xl ">{title?.slice(0, 70)}</h2></a></Link>
